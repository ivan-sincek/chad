...
import argparse, nagooglesearch, random, sys
...
class MyArgParser(argparse.ArgumentParser):
	...
	def print_help(self):
		...
		print("")
		print("Usage:   chad -q queries     [-p playwright] [-s site         ] [-x proxies    ] [-o out         ]")
		print("Example: chad -q queries.txt [-p           ] [-s *.example.com] [-x proxies.txt] [-o results.json]")
		print("")
		print("DESCRIPTION")
		...
		print("PAGE RESULTS")
		print("    Number of results per page - deprecated and capped at 10 by Google")
		print("    Default: 10")
		print("    -pr, --page-results = 5 | etc.")
		...
		print("    Google frequently changes cookies, so default ones might not work")
	...
	def error(self, message):
		if len(sys.argv) > 1:
			print("Missing a mandatory option (-q) and/or optional (-s, -t, -tr, -pr, -min-q, -max-q, -min-p, -max-p, -p, -b, -a, -x, -dir, -th, -o, -nsos, -dbg)")
...
class Validate:
	...
	def __validate_maximum_queries(self):
		tmp = self.__args.minimum_queries + 30
	...
	def __validate_maximum_pages(self):
		tmp = self.__args.minimum_pages + 15
